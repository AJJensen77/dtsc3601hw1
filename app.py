...
def load_skaters_data():
    cols = ["playerId", "name", "team", "position", "situation", "games_played",
            "I_F_goals", "I_F_points", "I_F_primaryAssists", "I_F_secondaryAssists",
            "I_F_hits", "I_F_penalityMinutes", "I_F_shotsOnGoal", "I_F_xGoals",
            "onIce_corsiPercentage"]
    skaters = pd.read_csv(SKATERS_CSV, usecols=cols)
    skaters = skaters[skaters["situation"] == "all"].copy()
    skaters["assists"] = skaters["I_F_primaryAssists"] + skaters["I_F_secondaryAssists"]
    return skaters
